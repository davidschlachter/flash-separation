public class RiddersMethod extends RootFinder {
  ...
  public static double findRoot(Function func, double x1, double x2, double xacc, boolean verbose) {
    ...
    for(int j=0; j < JMAX; j++)
    {
      ...
      if (verbose == true) System.out.println("Ridders Method: xL="+xL+" f(xL)="+fL+", xM="+xM+" f(xM)="+fM+", xU="+xU+" f(xU)="+fU);
      ...
      if((j > 0) && (error < xacc)) {
        if (verbose == true) System.out.println("Root was found to be "+xR+" evaluated at "+fR);
        return xR;
      }
    }
    if (verbose == true) System.out.println("Root was found to be "+xR+" evaluated at "+func.testFunction(xR));
    return xR;
  }
}